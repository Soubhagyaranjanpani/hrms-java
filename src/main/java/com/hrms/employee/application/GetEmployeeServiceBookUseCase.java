...
public class GetEmployeeServiceBookUseCase {
    ...
    public EmployeeServiceBookResponse execute(Long employeeId) {
        ...
        res.setRole(emp.getRole() != null ? emp.getRole().getName() : null);
        ...
        res.setServiceHistory(
                historyRepo.findByEmployee_Id(employeeId)
                        ...
                        .map(h -> {
                            ...
                            d.setDesignation(
                                    h.getDesignation() != null
                                            ? h.getDesignation().getName()
                                            : null
                            );
        ...
        res.setSkills(
                skillRepo.findByEmployee_Id(employeeId)
                        ...
                        .map(s -> {
                            ...
                            d.setId(s.getId());
                            d.setEmployeeId(s.getEmployee().getId());
                            d.setSkillId(s.getSkill().getId());
                            d.setSkillName(s.getSkill().getName()); // adjust "getName()" to match Skill's actual field
                            return d;
                        })
                        .collect(Collectors.toList())
        );
